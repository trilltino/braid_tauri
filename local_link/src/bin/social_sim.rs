...
use local_link::backend::db::init_db;
use local_link::backend::messaging::{
    create_conversation_db, send_friend_request_db, send_message_db,
};
use local_link::backend::ServerState;
use local_link::backend::SharedState;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let alice = local_link::auth::AuthResponse {
        token: "f".to_string(),
    ...
    let bob = local_link::auth::AuthResponse {
        token: "f".to_string(),
    ...
    let msg1 = send_message_db(
        ...
        axum::extract::Json(local_link::backend::messaging::SendMessage {
