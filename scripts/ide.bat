...
:: Configuration
if exist "%~dp0braid_root.txt" (
    set /p BRAID_ROOT=<"%~dp0braid_root.txt"
) else (
    set "BRAID_ROOT=%~dp0braid_data"
)
...
echo ========================================
echo    LocalLink IDE Helper (Server + Daemon)
echo ========================================
...
start "CHAT SERVER" cmd /c "set RUST_LOG=%RUST_LOG%& set BRAID_ROOT=%BRAID_ROOT%& cd /d "%~dp0" & echo === CHAT SERVER (port 3001) === & "%CARGO_TARGET_DIR%\release\local_link_server.exe" || pause"

timeout /t 2 /nobreak >nul

echo.
echo ========================================
echo    LocalLink IDE Helper - ALL RUNNING!
echo ========================================
echo.
echo [SERVICES]
