...
set "RUST_LOG=info,braid_core=debug,local_link=debug,server=debug"

echo ========================================
echo   LocalLink Full Local Test (2 Clients)
echo ========================================
echo.
...
if not exist "%CARGO_TARGET_DIR%\release\local_link_server.exe" (
...
start "CHAT SERVER A" cmd /c "set RUST_LOG=%RUST_LOG%& set BRAID_ROOT=%~dp0client_a& set SERVER_PORT=3001& cd /d "%~dp0" & echo === CHAT SERVER A (port 3001) === & "%CARGO_TARGET_DIR%\release\local_link_server.exe" || pause"
...
start "Client A UI" cmd /c "cd /d "%~dp0..\local_link" && set BRAID_ROOT=%~dp0client_a&& set CHAT_SERVER_URL=http://localhost:3001&& npm run tauri dev -- --port 8081"
...
start "Client B UI" cmd /c "cd /d "%~dp0..\local_link" && set BRAID_ROOT=%~dp0client_b&& set CHAT_SERVER_URL=http://localhost:3002&& npm run tauri dev -- --port 8082"

echo.
echo ========================================
echo   Two Full LocalLink Stacks Started!
echo ========================================
echo.
echo CLIENT A:
