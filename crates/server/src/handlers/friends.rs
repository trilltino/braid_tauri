...
use crate::friends::{FriendRequest, Contact};
...
pub async fn send_friend_request(
    ...
    match state.friends.send_request(user.id, req.to_email, req.message).await {
        Ok(request) => {
            info!("Friend request sent from {} to {}", user.username, request.to_email);
            Ok(Json(request))
        }
        Err(e) => {
...
pub async fn respond_friend_request(
    ...
    match state.friends.respond_to_request(&request_id, accept).await {
        Ok(_) => {
            info!("Friend request {} {}", request_id, if accept { "accepted" } else { "rejected" });
            Ok(StatusCode::OK)
        }
        Err(e) => {
            Err((StatusCode::BAD_REQUEST, Json(ErrorResponse {
