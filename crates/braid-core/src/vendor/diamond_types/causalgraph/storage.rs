...
use crate::vendor::diamond_types::encoding::cg_entry::{
    read_cg_entry_into_cg_nonoverlapping, write_cg_entry,
};
...
impl CGStorage {
    pub fn open<P: AsRef<Path>>(path: P) -> Result<(CausalGraph, CGStorage), CGError> {
        ...
        if !active_blit.data.is_empty() {
            ...
            let id_p =
                read_cg_entry_into_cg_nonoverlapping(&mut reader, false, &mut cg, &mut read_map)?;
            cgs.entry = id_p;
        ...
        debug_assert_eq!(
            cgs.file.stream_position()?,
            cgs.data_start() + committed_filesize
        );

        Ok((cg, cgs))
    }

    fn read_initial_blits<'a>(
        &mut self,
        raw_buf: &'a mut [u8; MAX_BLIT_SIZE * 2],
        blit_size: u64,
    ) -> Result<Blit<'a>, io::Error> {
    ...
    fn write_blit(&mut self, blit: Blit) -> Result<(), CGError> {
        debug_assert_eq!(
            ...
            self.next_write_location + self.data_start()
        );
    ...
    pub(crate) fn push_entry_no_sync(
        &mut self,
        bump: &Bump,
        entry: CGEntry,
        aa: &AgentAssignment,
