...
use crate::vendor::rle::{AppendRle, SplitableSpan};
...
use crate::vendor::diamond_types::frontier::{debug_assert_sorted, FrontierRef};
use crate::vendor::diamond_types::{Frontier, LV};
...
impl Graph {
    ...
    pub(crate) fn frontier_contains_version(&self, frontier: &[LV], target: LV) -> bool {
        ...
        while let Some(order) = queue.pop() {
            ...
            if entry.shadow_contains(target) {
                return true;
            }
    ...
    pub fn frontier_contains_frontier(&self, a: &[LV], b: &[LV]) -> bool {
        if a == b {
            return true;
        } // Might be a pointless optimization.

        for bb in b {
            if !self.frontier_contains_version(a, *bb) {
                return false;
            }
        }
        true
    ...
    pub fn diff_rev(&self, a: &[LV], b: &[LV]) -> DiffResult {
        ...
        if a.len() == 1 && b.len() == 1 {
            ...
            if self.is_direct_descendant_coarse(a, b) {
                // a >= b.
                return (
                    smallvec![(b.wrapping_add(1)..a.wrapping_add(1)).into()],
                    smallvec![],
                );
            ...
            if self.is_direct_descendant_coarse(b, a) {
                ...
                return (
                    smallvec![],
                    smallvec![(a.wrapping_add(1)..b.wrapping_add(1)).into()],
                );
        ...
        V: FnMut(DTRange, DiffFlag),
    {
        ...
        let frontier: Frontier = 'outer: loop {
            ...
            while let Some((peek_time, peek_flag)) = queue.peek() {
                if *peek_time == time {
                    // NOTE: This compares the whole frontier!
            ...
            let mut range = DTRange {
                start: containing_txn.span.start,
                end: t + 1,
            };
...
impl Graph {
    ...
    pub fn find_dominators_wide_rev(&self, versions: &[LV]) -> SmallVec<LV, 2> {
        ...
        if last_entry.shadow <= min_v {
            return smallvec![max_v];
        }
        ...
        if v_1.len() == 1 && v_2.len() == 1 {
            ...
            return match self.version_cmp(a, b) {
                None => {
                    ...
                    if a < b {
                        Frontier::from_sorted(&[a, b])
                    } else {
                        Frontier::from_sorted(&[b, a])
                    }
                }
    ...
    fn find_dominators_full_internal<F, I>(
        &self,
        versions_iter: I,
        stop_at_shadow: usize,
        mut visit: F,
    ) where
        F: FnMut(LV, bool),
        I: Iterator<Item = LV>,
...
pub mod test {
    ...
    use crate::vendor::rle::{AppendRle, MergableSpan};

    use crate::vendor::diamond_types::causalgraph::graph::tools::DiffFlag::*;
    use crate::vendor::diamond_types::causalgraph::graph::tools::{DiffFlag, DiffResult};
    use crate::vendor::diamond_types::causalgraph::graph::*;
    use crate::vendor::diamond_types::dtrange::DTRange;
    use crate::vendor::diamond_types::frontier::debug_assert_sorted;
    use crate::vendor::diamond_types::{Frontier, LV};
    ...
    fn assert_conflicting(
        ...
        #[cfg(feature = "gen_test_data")]
        {
            ...
            let mut f = std::fs::File::options()
                ...
                .append(true)
                .create(true)
                .open("test_data/causal_graph/conflicting.json")
                .unwrap();
    ...
    fn assert_frontier_cmp(graph: &Graph, a: &[LV], b: &[LV], expected: Option<Ordering>) {
        if a == b {
            assert_eq!(expected, Some(Ordering::Equal))
        }
    ...
    fn assert_frontier_contains_version(
        graph: &Graph,
        frontier: &[LV],
        target: LV,
        expected: bool,
        ...
        #[cfg(feature = "gen_test_data")]
        {
            ...
            let mut f = std::fs::File::options()
                ...
                .append(true)
                .create(true)
                .open("test_data/causal_graph/version_contains.json")
                .unwrap();
            writeln!(f, "{}", serde_json::to_string(&t).unwrap()).unwrap();
        }
        ...
        assert_eq!(
            graph.frontier_contains_frontier(frontier, &[target]),
            expected
        );

        assert_eq!(
            graph.frontier_cmp(frontier, frontier),
            Some(Ordering::Equal)
        );
        // dbg!(frontier, &[target]);
        if expected {
    ...
    fn assert_diff_eq(
        ...
        #[cfg(feature = "gen_test_data")]
        {
            ...
            let mut f = std::fs::File::options()
                ...
                .append(true)
                .create(true)
                .open("test_data/causal_graph/diff.json")
                .unwrap();
            writeln!(f, "{}", serde_json::to_string(&t).unwrap()).unwrap();
        }
    ...
    fn common_item_smoke_test() {
        ...
        assert_conflicting(
            ...
            &[6],
            &[2],
            &[(0..2, Shared), (2..3, OnlyB), (3..5, OnlyA), (6..7, OnlyA)],
            &[],
        );
        assert_conflicting(
            &graph,
            &[6],
            &[5],
            &[(0..2, OnlyA), (3..5, Shared), (5..6, OnlyB), (6..7, OnlyA)],
            &[],
        ); // 6 includes 1, 0.
        assert_conflicting(
            &graph,
            &[5, 6],
            &[5],
            &[(0..2, OnlyA), (3..6, Shared), (6..7, OnlyA)],
            &[],
        );
        assert_conflicting(
            &graph,
            &[5, 6],
            &[2],
            &[(0..2, Shared), (2..3, OnlyB), (3..7, OnlyA)],
            &[],
        );
        assert_conflicting(
            &graph,
            &[2, 6],
            &[5],
            &[(0..3, OnlyA), (3..5, Shared), (5..6, OnlyB), (6..7, OnlyA)],
            &[],
        );
        ...
        assert_conflicting(
            ...
            &[9],
            &[2, 7],
            &[(0..5, Shared), (6..8, Shared), (8..10, OnlyA)],
            &[],
        );
    }
    ...
    fn check_dominators(graph: &Graph, input: &[LV], expected_yes: &[LV]) {
        ...
        let expected_no: Vec<_> = input
            .iter()
            .filter(|v| !expected_yes.contains(v))
            .copied()
            .collect();
