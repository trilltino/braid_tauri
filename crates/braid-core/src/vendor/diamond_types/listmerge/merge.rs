...
use crate::vendor::rle::{
    AppendRle, HasLength, MergableSpan, MergeableIterator, Searchable, SplitableSpanCtx, Trim,
    TrimCtx,
};
...
use crate::vendor::diamond_types::listmerge::merge::TransformedResult::{
    BaseMoved, DeleteAlreadyHappened,
};
...
use crate::vendor::diamond_types::ost::content_tree::{
    Content, ContentCursor, ContentTree, DeltaCursor,
};
...
impl M2Tracker {
    pub(super) fn new() -> Self {
        ...
        result
            .range_tree
            .set_single_item_notify(underwater, notify_for(&mut result.index));
    ...
    pub(super) fn clear(&mut self) {
        ...
        self.range_tree
            .set_single_item_notify(underwater, notify_for(&mut self.index));
    }

    pub(super) fn marker_at(&self, lv: LV) -> LeafIdx {
        let marker = self.index.get_entry(lv).val;
        let Marker::InsPtr(leaf) = marker else {
            panic!("No marker at lv")
        };
        leaf
    }
    ...
    fn get_cursor_after(&self, lv: LV, stick_end: bool) -> ContentCursor {
        if lv == usize::MAX {
            ...
            if !stick_end {
                cursor.roll_next_item(&self.range_tree);
            }
            cursor
        }
    }
    ...
    pub(super) fn integrate(
        ...
        loop {
            if dc.0.offset > 0 // If cursor > 0, the item we're on now is INSERTED.
                || !dc.roll_next_item(&mut self.range_tree)
            {
                // End of the document
                break;
            }
            ...
            match other_left_cursor.cmp(&left_cursor, &self.range_tree) {
                ...
                Ordering::Equal => {
                    if item.origin_right == other_entry.origin_right {
                        ...
                        if other_right_cursor.cmp(&my_right_cursor, &self.range_tree)
                            == Ordering::Less
                        {
                            if !scanning {
                                scanning = true;
            ...
            cursor_pos.cur += if item.takes_up_space::<true>() {
                item.len() - offset
            } else {
                0
            };
            cursor_pos.end += if item.takes_up_space::<false>() {
                item.len() - offset
            } else {
                0
            };
        ...
        self.range_tree
            .insert(item, &mut dc, true, &mut notify_for(&mut self.index));
    ...
    pub(super) fn apply(
        &mut self,
        aa: &AgentAssignment,
        _ctx: &ListOperationCtx,
        op_pair: &KVPair<ListOpMetrics>,
        max_len: usize,
        agent: AgentId,
    ) -> (usize, TransformedResult) {
        ...
        match op.kind {
            ListOpKind::Ins => {
                if !op.loc.fwd {
                    // Fallback for reverse insertions: apply each char individually.
                    // This is slightly slower but avoids the panic and ensures correctness.
                    let mut xf_pos = 0;
                    for i in (0..len).rev() {
                        let mut sub_op = op_pair.clone();
                        sub_op.truncate_ctx(i + 1, _ctx);
                        let sub_op = sub_op.truncate_keeping_right_ctx(i, _ctx);
                        let (len_here, res) = self.apply(aa, _ctx, &sub_op, 1, agent);
                        debug_assert_eq!(len_here, 1);
                        if let BaseMoved(p) = res {
                            if i == 0 {
                                xf_pos = p;
                            }
                        }
                    }
                    return (len, BaseMoved(xf_pos));
                }
                ...
                let (origin_left, end_pos, mut new_cursor) = if op.start() == 0 {
                    ...
                    let (mut end_pos, mut cursor) =
                        self.range_tree.mut_cursor_before_cur_pos(op.start() - 1);
            ...
            ListOpKind::Del => {
                ...
                self.index.set_range(
                    (lv_start..lv_start + len).into(),
                    Marker::Del(DelRange {
...
impl<'a> TransformedOpsIterRaw<'a> {
    ...
    fn next_from(
        aa: &AgentAssignment,
        tracker: &mut M2Tracker,
        op_ctx: &ListOperationCtx,
        mut pair: KVPair<ListOpMetrics>,
        ...
        let result = match xf_result {
            BaseMoved(xf_pos) => TransformedResultRaw::Apply { xf_pos, op: pair },
        ...
        if let Some(op_iter) = self.op_iter.as_mut() {
            if let Some(pair) = op_iter.next() {
                ...
                let (remainder, result) =
                    Self::next_from(self.aa, &mut self.tracker, self.op_ctx, pair);
                if let Some(r) = remainder {
                    op_iter.push_back(r);
                }
                return Some(result);
            ...
            match action {
                ...
                M1PlanAction::Apply(span) => {
                    if !self.applying {
                        ...
                        self.tracker
                            .apply_range(self.aa, self.op_ctx, self.ops, *span, None);
                    } else {
                        let mut op_iter =
                            BufferedIter::new(OpMetricsIter::new(self.ops, self.op_ctx, *span));
                        if let Some(pair) = op_iter.next() {
                            let (remainder, result) =
                                Self::next_from(self.aa, &mut self.tracker, self.op_ctx, pair);
                            if let Some(r) = remainder {
...
impl<'a> Iterator for TransformedSimpleOpsIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(TransformedResultRaw::DeleteAlreadyHappened(range)) => {
                Some(DeleteAlreadyHappened(range))
            }
        }
    }
}
...
impl TextInfo {
    pub(crate) fn get_xf_operations_full<'a>(
        &'a self,
        subgraph: &'a Graph,
        aa: &'a AgentAssignment,
        from: &[LV],
        merging: &[LV],
    ) -> TransformedOpsIterRaw<'a> {
        TransformedOpsIterRaw::new(subgraph, aa, &self.ctx, &self.ops, from, merging)
    }

    pub(crate) fn with_xf_iter<F: FnOnce(TransformedOpsIterRaw, Frontier) -> R, R>(
        &self,
        cg: &CausalGraph,
        from: &[LV],
        merge_frontier: &[LV],
        ...
        let op_spans = self.ops.iter().map(|e| e.span()).rev().merge_spans_rev();
        ...
        let iter =
            rle_intersect_rev(op_spans, conflict.rev_spans.iter().copied()).map(|pair| pair.0);
        ...
        let merge_frontier = cg
            .graph
            .project_onto_subgraph_raw(iter.clone(), merge_frontier);

        // let mut iter = TransformedOpsIter::new(oplog, &self.frontier, merge_frontier);
        let iter = self.get_xf_operations_full(
            &subgraph,
            &cg.agent_assignment,
            from.as_ref(),
            merge_frontier.as_ref(),
        );
    ...
    pub fn xf_operations_from<'a>(
        &'a self,
        cg: &'a CausalGraph,
        from: &[LV],
        merging: &[LV],
    ) -> Vec<(DTRange, Option<TextOperation>)> {
...
mod test {
    ...
    use crate::vendor::diamond_types::listmerge::yjsspan::{
        deleted_n_state, SpanState, DELETED_ONCE,
    };
    ...
    fn items_state(tracker: &M2Tracker, filter_underwater: usize) -> Vec<(usize, SpanState)> {
        items(tracker, filter_underwater)
            .iter()
            .map(|i| (i.len(), i.current_state))
            .collect()
    }
    ...
    fn test_concurrent_insert() {
        ...
        let i: Vec<_> = items(&t, 0)
            .iter()
            .map(|i| (i.id, i.current_state))
            .collect();
        assert_eq!(
            i,
            &[((0..3).into(), NOT_INSERTED_YET), ((3..6).into(), INSERTED),]
        );
        // dbg!(&t);
    ...
    fn dump_index_stats(bench_name: &str) {
        let mut bytes = vec![];
        File::open(format!("benchmark_data/{bench_name}.dt"))
            .unwrap()
    ...
    fn print_stats() {
        ...
        File::open("benchmark_data/git-makefile.dt")
            .unwrap()
            .read_to_end(&mut bytes)
            .unwrap();
        let o = ListOpLog::load_from(&bytes).unwrap();
        ...
        println!("----");
        bytes.clear();
        File::open("benchmark_data/node_nodecc.dt")
            .unwrap()
            .read_to_end(&mut bytes)
            .unwrap();
        let o = ListOpLog::load_from(&bytes).unwrap();
