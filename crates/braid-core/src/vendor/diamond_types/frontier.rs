...
pub(crate) fn is_sorted_iter<
    const EXPECT_UNIQ: bool,
    V: Ord + Eq + Debug,
    I: Iterator<Item = V>,
>(
    mut iter: I,
...
pub(crate) fn is_sorted_slice<const EXPECT_UNIQ: bool, V: Ord + Eq + Debug + Copy>(
    slice: &[V],
) -> bool {
    if slice.len() >= 2 {
...
impl Frontier {
    ...
    pub fn advance_sparse(&mut self, graph: &Graph, range: DTRange) {
        let Ok(txn_idx) = graph.entries.find_index(range.start) else {
            return;
        };
        // let txn_idx = graph.entries.find_index(range.start).unwrap();
    ...
    fn insert_nonoverlapping(&mut self, new_item: LV) {
        ...
        // Binary search might actually be slower here than a linear scan.
        if let Err(new_idx) = self.0.binary_search(&new_item) {
    ...
    pub fn insert(&mut self, new_item: LV) {
        ...
        // frontier.
        let Err(new_idx) = self.0.binary_search(&new_item) else {
            return;
        };
        self.0.insert(new_idx, new_item);
        self.debug_check_sorted();
...
mod test {
    ...
    fn frontier_movement_smoke_tests() {
        ...
        let graph = Graph::from_simple_items(&[GraphEntrySimple {
            span: (0..10).into(),
